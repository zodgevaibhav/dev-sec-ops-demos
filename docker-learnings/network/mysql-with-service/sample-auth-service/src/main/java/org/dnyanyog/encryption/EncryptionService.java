...
public class EncryptionService {
  ...
  private static final String SECRET_KEY =
      "5F270B070EF2F0BAB8123A810368B0E4"; // Encryption key, should be store at
  // secure location
  private static final String ALGORITHM =
      "AES"; // Encryption algorithm, should be store at secure location
  ...
  static {
    secretKey =
        new SecretKeySpec(
            SECRET_KEY.getBytes(StandardCharsets.UTF_8), ALGORITHM); // Create secret key
    // object from the key
    try {
  ...
  public static String decrypt(String encryptedData) throws Exception {
    byte[] decryptedData =
        cipher.doFinal(Base64.getDecoder().decode(encryptedData)); // Decrypt data in to bytes
